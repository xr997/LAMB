...
推荐生成一张 **科研论文风格的系统框架图**，用于 README 首图、比赛答辩封面或项目海报。画面可以有现代感，但整体应像论文中的方法框架图，而不是商业广告海报。

推荐比例：`16:9` 或 `4:3`。

```text
绘制一张科研论文风格的系统框架图，用于介绍开源项目 LAMB。整体像 ACL、CHI、ICSE、USENIX 论文中的方法框架图，清晰、克制、模块化，但不要过于严肃，可以有轻微现代产品感。画面从左到右展示 LAMB 的完整流程：输入侧是本地资料夹，里面包含 PDF、Word、Markdown、TXT、CSV、JSON、论文、作业、简历、会议纪要等文档；中间是“LAMB 核心”大模块，内部包含六个子模块：“文档扫描”“文本解析”“安全守卫”“长文档分块”“大模型工作流”“结果导出”；“安全守卫”模块用盾牌或警示标记表现 Prompt 注入检测、敏感信息脱敏、路径边界保护；“长文档分块”模块表现长文档被切分成多个片段；“大模型工作流”模块分出三条支路：批量处理、多文档问答、字段抽取；右侧输出研究报告、CSV/JSON 表格、逐文件摘要、审计清单。图中使用论文框架图常见的方框、箭头、分组边框、简洁图标和少量颜色强调，白色或浅灰背景，蓝色和青绿色为主，橙色用于安全风险提示。风格要求：科研论文方法图、系统架构图、干净的矢量插画、高可读性、间距均衡、装饰克制、轻微阴影；不要真人照片，不要卡通动物，不要赛博朋克风格，不要杂乱背景，不要随机乱码文字，不要真实公司标志。顶部预留标题区域，可写 “LAMB：安全可信的多文档 AI 批处理助理”。
```

如果生成工具更适合英文 prompt，可以使用英文版 README 中的对应提示词：[README.en.md](README.en.md#project-figure-prompt)。
